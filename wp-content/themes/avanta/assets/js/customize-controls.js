/* global avantaBgColors, avantaColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( avantaBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( avantaBgColors[ context ].color ) {
						backgroundColorValue = avantaBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( avantaBgColors[ context ].setting ).get();
					}
					avantaSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( avantaBgColors ).forEach( function( context ) {
			wp.customize( avantaBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					avantaSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		avantaSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				avantaSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function avantaSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = avantaColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function avantaSetRetineLogoVisibility( visible ) {
